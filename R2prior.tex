...
rstanarm} package. The {\tt stan\_lm} function will use Stan \cite{stan} to draw
from the posterior distribution of the primitive parameters via MCMC and then
use the equations in Section~\ref{sec:posterior} to obtain the estimates to be
returned to the user. To fit the model we simply prepend {\tt stan\_} to the
