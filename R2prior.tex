...
about $\sigmaEps$ are implied by our beliefs about $\omega$ and \Rsq via the
relations
$$\sigmaEps = \sigma_y \sqrt{1 - R^2} = \omega s_y \sqrt{1 - R^2}.$$
Thus, the only remaining distribution to specify is the prior for the model
intercept
...
$f_\alpha$ are certainly possible, for instance a zero-mean normal
...
of size $S$ from $f_{post}$ consisting of draws
$$\left(\phi, \alpha, \u, R^2\right)^{(s)}, \quad s = 1, \dots, S$$
and we wish to make inferences using the parameters of interest
$\boldbeta$, $\sigma_y$, and $\sigmaEps$. Following our definitions,
we can obtain a sample from the posterior of
$\left(\boldbeta, \sigma_y, \sigmaEps\right)$ from the MCMC draws of the
primitive parameters by computing
%
\begin{align*}
\sigma_y^{(s)} &= \omega^{(s)} s_y = e^{\phi^{(s)}} s_y \\
\sigmaEps^{(s)} &= \sigma_y^{(s)} \sqrt{1 - {R^2}^{(s)}} \\
\boldbeta^{(s)} &= \R^{-1}\, \u^{(s)} \, \sigma_y^{(s)}
                      \sqrt{{R^2}^{(s)} \left(N-1\right)}
\end{align*}
%
