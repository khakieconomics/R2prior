...
This is only valid for $K \geq 2$ as otherwise the mode does not exist.
...
given a prior value for the left-hand side it is easy to numerically solve for
...
hyperparameter$\eta$. For larger values of $\eta$ the prior is more concentrated
around zero.}
\label{fig:betaplot}
\end{figure}
...
outcome and $\omega > 0$ is an unknown scale parameter to be estimated. The only
prior for $\omega$ that does not contravene Bayes' theorem in this situation is
the Jeffreys prior,
...
When implementing the model, we prefer to instead work with $\omega$ on the log
scale. A flat prior $f_\phi(\phi) \propto 1$, where $\phi = \log{\omega}$, is
equivalent to the Jeffreys prior on $\omega$ itself. We refer to $\phi$ as the
\emph{log fit-ratio} since
%
$$\phi = \log{\omega} = \log{\frac{\sigma_y}{s_y}}$$
%
is the logarithm of the ratio of the marginal standard deviation of the outcome
\emph{implied by the model} to the observed sample standard deviation.
Consequently, we can interpret the log fit-ratio as a measure of underfitting or
overfitting.
...
The previous sections imply a joint posterior distribution $f_{\em post}$
for the primitive parameters $\Theta = \left(\phi, \alpha, \u, r^2 \right)$,
%
$$
f_{\em post}\left(\Theta \given \y, \X = \Q\R, \eta \right)
\propto \mathcal{L}\left(\y \given \Q, \Theta \right)
\times f_{\Theta|K,\eta}\left(\Theta \given K, \eta \right),
$$
where the joint prior $f_{\Theta|K,\eta}$ factors as
$$f_\phi(\phi) f_\alpha(\alpha) f_{\u}(\u) f_{r^2}(r^2 | K, \eta).$$
and $\mathcal{L}$ is the likelihood (using the QR-reparameterization) for the
entire sample.

As demonstrated in Section~\ref{sec:example}, draws from the posterior can be
obtained via Markov chain Monte Carlo (MCMC), after which the parameters of
